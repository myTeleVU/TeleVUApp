...
export default function AddScreenToCall ({
  open,
  handleClose,
  online,
  addToCall,
  ...
  React.useEffect(() => {
    ...
    if (allUsers) {
      allUsers.forEach((item) => {
        if (item.email !== me.email && item.userType === 'ScreenVU') {
  ...
  React.useEffect(async () => {
    if (online[portal.name]) {
      setOnlineOperators(online[portal.name].ScreenVU)
    } else {
      setOnlineOperators([])
    }
  }, [portal.name, online])
  ...
  return (
    <AddToCall
      ...
      title={theme.SCREENVU}
    />
  )
}

AddScreenToCall.propTypes = {
  open: PropTypes.bool,
